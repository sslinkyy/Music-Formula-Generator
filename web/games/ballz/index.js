...
export function buildBallzGameDialog(onFinish, options = {}) {
  ...
  scoreDiv.textContent = 'Balls: 1 | Level: 1 | Score: 0';
  ...
  hint.textContent = 'Move mouse/touch to aim. Click/tap to launch balls. Collect ball powerups and destroy bricks!';
  ...
  function initLevel() {
    bricks = [];
    const numBricks = Math.min(3 + level, GRID_COLS);
    const maxHits = Math.min(level, 20);

    // Add bricks to top row
    for (let i = 0; i < numBricks; i++) {
      const col = Math.floor(Math.random() * GRID_COLS);
      const existing = bricks.find(b => b.row === 0 && b.col === col);
      if (!existing) {
        // 20% chance to be a ball collectible
        if (Math.random() < 0.2) {
          bricks.push({
            row: 0,
            col: col,
  ...
  function dropBricks() {
    ...
    // Check game over
    const bottomRow = bricks.find(b => !b.isBall && b.row >= GRID_ROWS - 1);
    if (bottomRow) {
      endGame();
    }
  }
  ...
  function drawBricks() {
    bricks.forEach(brick => {
      ...
      if (brick.isBall) {
        ...
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.moveTo(x + BRICK_SIZE / 2, y + BRICK_SIZE / 2 - 8);
        ctx.lineTo(x + BRICK_SIZE / 2, y + BRICK_SIZE / 2 + 8);
        ctx.moveTo(x + BRICK_SIZE / 2 - 8, y + BRICK_SIZE / 2);
        ctx.lineTo(x + BRICK_SIZE / 2 + 8, y + BRICK_SIZE / 2);
        ctx.stroke();
      } else {
        // Draw numbered brick
        const hue = (brick.hits / level) * 120; // Red (0) to green (120)
  ...
  function draw() {
    ...
    for (let i = 0; i <= GRID_ROWS; i++) {
      ...
      ctx.lineTo(CANVAS_WIDTH, i * BRICK_SIZE);
      ctx.stroke();
    }
    ...
    scoreDiv.textContent = `Balls: ${ballCount} | Level: ${level} | Score: ${score}`;
  }
  ...
  function updateAim(clientX, clientY) {
    ...
    if (aimAngle > 0) {
      if (aimAngle < Math.PI / 2) {
        aimAngle = -Math.PI / 18; // 10 degrees right
      } else {
        aimAngle = -Math.PI + Math.PI / 18; // 10 degrees left
      }
    }

    // Update platform position to mouse X (only when not launching)
    if (!launchInProgress && balls.length === 0) {
      platformX = Math.max(20, Math.min(CANVAS_WIDTH - 20, mouseX));
    }
  }
  ...
  function endGame() {
    ...
    setTimeout(() => {
      const result = {
        genres: genres,
        premise: `Ballz level ${level} complete`,
