...
export function buildShooterGameDialog(onFinish, options = {}) {
  ...
  const W = 800, H = 420; // logical pixels
  ...
  const player = { x: W/2, y: H/2, vx: 0, vy: 0, speed: 180*speedScale, r: 12, hp: hpByDiff[difficulty] || 3 };
  ...
  let joyActive = false, joyVecX = 0, joyVecY = 0, fireTouch = false;
  ...
  function update(dt){ pollGamepad(); const sp=player.speed*dt; let mvx = (keys['ArrowRight']||keys['d']?1:0) - (keys['ArrowLeft']||keys['a']?1:0); let mvy = (keys['ArrowDown']||keys['s']?1:0) - (keys['ArrowUp']||keys['w']?1:0); if (isMobileUI && joyActive) { const d=Math.hypot(joyVecX,joyVecY)||1; mvx = joyVecX/d; mvy = joyVecY/d; } else if (touchActive) { const dx = touchX - player.x; const dy = touchY - player.y; const d = Math.hypot(dx,dy)||1; mvx = dx/d; mvy = dy/d; mouseDown = true; } const len=Math.hypot(mvx,mvy)||1; player.x += (mvx/len)*sp; player.y += (mvy/len)*sp; clampPlayer(); const firing = (keys[' ']||mouseDown||(isMobileUI&&fireTouch)); if (firing && (now%200<16)) spawnBullet(); if (Math.random() < 0.03*spawnScale) spawnEnemy(); if (Math.random() < 0.015*spawnScale) spawnPickup(); if (Math.random() < 0.01*spawnScale) spawnHazard(); for (let i=bullets.length-1;i>=0;i--){ const b=bullets[i]; b.x+=b.vx*dt; b.y+=b.vy*dt; if (b.x<0||b.y<0||b.x>W||b.y>H) bullets.splice(i,1);} enemies.forEach(e=>{ const dx=player.x-e.x, dy=player.y-e.y; const d=Math.hypot(dx,dy)||1; const v=(60*speedScale)/(0.5+Math.random()); e.x+=(dx/d)*v*dt; e.y+=(dy/d)*v*dt; }); for (let i=enemies.length-1;i>=0;i--){ const e=enemies[i]; let dead=false; for (let j=bullets.length-1;j>=0;j--){ const b=bullets[j]; if (Math.hypot(b.x-e.x,b.y-e.y)<=e.r){ bullets.splice(j,1); dead=true; hits++; playSfx('hit'); spawnBurst(e.x,e.y,colorForKind(e.kind)); break; } } if (dead){ enemies.splice(i,1); kills++; combo++; const mult = 1 + Math.floor(combo/10); score += 100 * mult; bestCombo=Math.max(bestCombo,combo); genreKills[e.kind]=(genreKills[e.kind]||0)+1; if (Math.random()<0.35) spawnPickupAt(e.x,e.y); } } for (let i=hazards.length-1;i>=0;i--){ const h=hazards[i]; h.x+=h.vx*dt; h.y+=h.vy*dt; if (Math.hypot(h.x-player.x,h.y-player.y)<=h.r+player.r){ hazards.splice(i,1); player.hp--; combo=0; score = Math.max(0, score - 50); forbidden.add('hazards'); playSfx('hazard'); spawnBurst(player.x,player.y,'#FF6B6B'); } else if (h.x<0||h.y<0||h.x>W||h.y>H) hazards.splice(i,1);} for (let i=pickups.length-1;i>=0;i--){ const p=pickups[i]; if (Math.hypot(p.x-player.x,p.y-player.y)<=18){ pickups.splice(i,1); if (p.type==='tag') styleTags.add(p.label); else if (p.type==='kw') keywords.add(p.label); else if (p.type==='heal') player.hp=Math.min((hpByDiff[diffSel.value]||3),player.hp+1); score += 10; playSfx('pickup'); spawnBurst(p.x,p.y, p.type==='tag'?'#6BCB77':'#B084CC'); } } particles.forEach(pt=>{ pt.x+=pt.vx*dt; pt.y+=pt.vy*dt; pt.life-=dt; }); for(let i=particles.length-1;i>=0;i--) if (particles[i].life<=0) particles.splice(i,1); }
  function clampPlayer(){ player.x=Math.max(player.r,Math.min(W-player.r,player.x)); player.y=Math.max(player.r,Math.min(H-player.r,player.y)); }
  ...
  function spawnEnemy(){ const side=Math.floor(Math.random()*4); const r=10+Math.random()*10; let x=0,y=0; if(side===0){x=0;y=Math.random()*H;} else if(side===1){x=W;y=Math.random()*H;} else if(side===2){x=Math.random()*W;y=0;} else {x=Math.random()*W;y=H;} const kind = enemyKinds[Math.floor(Math.random()*enemyKinds.length)]; enemies.push({x,y,vx:0,vy:0,r,kind}); }
  function spawnHazard(){ const r=10+Math.random()*8; const x=Math.random()*W, y=Math.random()*H; const ang=Math.random()*Math.PI*2; const v=80*speedScale; hazards.push({ x,y,vx:Math.cos(ang)*v,vy:Math.sin(ang)*v,r }); }
  function spawnPickup(){ const type = Math.random()<0.6 ? 'tag' : 'kw'; const label = type==='tag' ? sampleTag() : sampleKeyword(); const x=Math.random()*W, y=Math.random()*H; pickups.push({ x,y,type,label }); }
  ...
  // Pointer (touch) support + mobile overlays
  const toLocal = (e) => { const r = canvas.getBoundingClientRect(); const x=((e.clientX - r.left)/r.width)*W; const y=((e.clientY - r.top)/r.height)*H; return {x,y}; };
  canvas.addEventListener('pointerdown', (e)=>{ try{ if(e.cancelable) e.preventDefault(); }catch(_){} touchActive=true; const p=toLocal(e); touchX=p.x; touchY=p.y; mouseDown=true; }, {passive:false});
  canvas.addEventListener('pointermove', (e)=>{ if(!touchActive) return; try{ if(e.cancelable) e.preventDefault(); }catch(_){} const p=toLocal(e); touchX=p.x; touchY=p.y; }, {passive:false});
  const endPtr=(e)=>{ try{ if(e && e.cancelable) e.preventDefault(); }catch(_){} touchActive=false; mouseDown=false; };
  canvas.addEventListener('pointerup', endPtr, {passive:false}); canvas.addEventListener('pointercancel', endPtr, {passive:false});
  if (isMobileUI) {
    const joy = document.createElement('div'); joy.style.position='absolute'; joy.style.left='12px'; joy.style.bottom='12px'; joy.style.width='120px'; joy.style.height='120px'; joy.style.borderRadius='999px'; joy.style.background='rgba(255,255,255,0.06)'; joy.style.border='1px solid rgba(255,255,255,0.2)'; joy.style.touchAction='none';
    const thumb=document.createElement('div'); thumb.style.position='absolute'; thumb.style.left='50%'; thumb.style.top='50%'; thumb.style.width='44px'; thumb.style.height='44px'; thumb.style.marginLeft='-22px'; thumb.style.marginTop='-22px'; thumb.style.borderRadius='999px'; thumb.style.background='rgba(255,255,255,0.18)'; thumb.style.border='1px solid rgba(255,255,255,0.2)'; joy.appendChild(thumb); frame.appendChild(joy);
    const jC=()=>{ const r=joy.getBoundingClientRect(); return {cx:r.left+r.width/2, cy:r.top+r.height/2, rr: Math.min(r.width,r.height)/2 - 4}; };
    const jM=(e)=>{ const {cx,cy,rr}=jC(); const dx=e.clientX-cx, dy=e.clientY-cy; const d=Math.hypot(dx,dy)||1; const cl=Math.min(d,rr); const nx=dx/d, ny=dy/d; joyVecX=nx; joyVecY=ny; thumb.style.transform=`translate(${nx*cl}px, ${ny*cl}px)`; };
    joy.addEventListener('pointerdown',(e)=>{ try{ if(e.cancelable) e.preventDefault(); }catch(_){} joyActive=true; jM(e); }, {passive:false});
    joy.addEventListener('pointermove',(e)=>{ if(!joyActive) return; try{ if(e.cancelable) e.preventDefault(); }catch(_){} jM(e); }, {passive:false});
    const jE=(e)=>{ try{ if(e && e.cancelable) e.preventDefault(); }catch(_){} joyActive=false; joyVecX=0; joyVecY=0; thumb.style.transform='translate(0,0)'; };
    joy.addEventListener('pointerup', jE, {passive:false}); joy.addEventListener('pointercancel', jE, {passive:false});
    const fire = document.createElement('div'); fire.style.position='absolute'; fire.style.right='16px'; fire.style.bottom='28px'; fire.style.width='78px'; fire.style.height='78px'; fire.style.borderRadius='999px'; fire.style.background='rgba(255,215,0,0.12)'; fire.style.border='1px solid rgba(255,215,0,0.35)'; fire.style.boxShadow='0 0 12px rgba(255,215,0,0.2) inset'; fire.style.touchAction='none'; fire.style.display='grid'; fire.style.placeItems='center'; fire.style.color='#ffd93d'; fire.style.fontSize='20px'; fire.textContent='🔥'; frame.appendChild(fire);
    fire.addEventListener('pointerdown',(e)=>{ try{ if(e.cancelable) e.preventDefault(); }catch(_){} fireTouch=true; }, {passive:false});
    const fE=(e)=>{ try{ if(e && e.cancelable) e.preventDefault(); }catch(_){} fireTouch=false; };
    fire.addEventListener('pointerup', fE, {passive:false}); fire.addEventListener('pointercancel', fE, {passive:false});
  }
  ...
  function drawPaused(){ ctx.fillStyle='#00000080'; ctx.fillRect(0,0,W,H); ctx.fillStyle='#ffffff'; ctx.font='bold 20px system-ui'; ctx.fillText('Paused', W/2 - 34, H/2); }
