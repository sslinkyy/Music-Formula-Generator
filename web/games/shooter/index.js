...
export function buildShooterGameDialog(onFinish, options = {}) {
  ...
  controls.appendChild(startBtn); controls.appendChild(restartBtn); controls.appendChild(quitBtn); controls.appendChild(rerollBtn);
  ...
  function reset(){ enemies.length=0; bullets.length=0; pickups.length=0; hazards.length=0; particles.length=0; player.x=canvas.width/2; player.y=canvas.height/2; player.vx=player.vy=0; player.hp=hpByDiff[diffSel.value]||3; shots=hits=kills=bestCombo=combo=0; Object.keys(genreKills).forEach(k=>delete genreKills[k]); styleTags.clear(); keywords.clear(); forbidden.clear(); }
  ...
  function draw(elapsed){ ctx.clearRect(0,0,canvas.width,canvas.height); // subtle background grid
    ...
    // hud
    ctx.fillStyle='#9aa3b2'; ctx.font='12px system-ui'; ctx.fillText(`Time: ${Math.max(0,(duration-elapsed)|0)}  HP: ${player.hp}  Kills: ${kills}  Combo: ${combo}  Acc: ${calcAcc()}%`, 8, 16); }
  ...
  function playSfx(type){ try { const C = window.__rgfAudioCtx || new (window.AudioContext||window.webkitAudioContext)(); window.__rgfAudioCtx = C; const o = C.createOscillator(); const g = C.createGain(); o.connect(g).connect(C.destination); let freq=440, dur=0.05, vol=0.06, typeW='sine'; if (type==='shoot'){ freq=660; dur=0.04; vol=0.05; typeW='square'; } else if (type==='hit'){ freq=220; dur=0.06; vol=0.06; typeW='sawtooth'; } else if (type==='pickup'){ freq=880; dur=0.07; vol=0.05; typeW='triangle'; } else if (type==='hazard'){ freq=140; dur=0.12; vol=0.07; typeW='sawtooth'; } g.gain.value=0; const nowT=C.currentTime; const attack=0.005, decay=dur; g.gain.setValueAtTime(0, nowT); g.gain.linearRampToValueAtTime(vol, nowT+attack); g.gain.exponentialRampToValueAtTime(0.0001, nowT+attack+decay); o.type=typeW; o.frequency.setValueAtTime(freq, nowT); o.start(nowT); o.stop(nowT+attack+decay+0.02); } catch(_){} }
